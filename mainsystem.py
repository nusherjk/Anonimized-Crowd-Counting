...
class DeepPrivacyConfig:
    ...
    opts = None
    config_path = None
    source_path = "./input_imgs/"
    target_path = "/output_imgs/"
    start_time = None
    end_time = None


def p2pNetInference( debug=False):
    ...
    img_to_draw = np.array(ann_imgs[0])
    ...
    cv2.imwrite(os.path.join(config.output_dir, 'pred{}.jpg'.format(predict_cnt)), img_to_draw)
