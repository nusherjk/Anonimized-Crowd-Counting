...
def get_anonimized(config):
    anonymizer, cfg = build_anonymizer(
        config.model, opts=config.opts, config_path=config.config_path,
        return_cfg=True)
    output_dir = cfg.output_dir
    source_paths = get_source_files(config.source_path)
    ...
    if len(video_paths) > 0:
        video_target_paths = get_target_paths(video_paths, config.target_path,
                                              output_dir)
    image_target_paths = []
    if len(image_paths) > 0:
        image_target_paths = get_target_paths(
            image_paths, config.target_path,
            output_dir)
    ...
    if len(image_paths) > 0:
        return anonymizer.anonymize_image_with_return(image_paths)
